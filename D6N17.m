...
curve_data:=GYData(6,17);
curve_quotients:=DataToQuotientList(curve_data);


P2<X,Y,T>:=ProjectiveSpace(Rationals(),2);
hyp_quotient:= Conic(P2,3*X^2 + 48*Y^2 + T^2);

wx:=iso<hyp_quotient -> hyp_quotient | [-X,Y,T], [-X,Y,T] >;
AutomorphismGroup(hyp_quotient,[wx]);
